...
*/
  ...
  ngOnInit(): void {
    this.log.info(`EntryForm initialization at ${Date()} with development mode ${isDevMode() ? "" : "NOT "}enabled`, this.id, this.id)
    this.log.verbose("EntryComponent - ngOnInit - Use settings to fill form", this.id, this.id)
 ...
 */
    ...
    this.entryDetailsForm.controls['locationFrmGrp'].valueChanges.subscribe(x => {
      this.log.verbose(`Subscription to locationFrmGrp got: ${x}`, this.id, this.id);
    })
    ...
    this.date = dayjs()
    this.log.verbose(`EntryForm ngOnInit completed at ${Date()}`, this.id)
  }
  ...
  */
  ...
  onFormSubmit(formData1: string): void {
    this.log.verbose(`Form submited at date=${this.date} `, this.id)
